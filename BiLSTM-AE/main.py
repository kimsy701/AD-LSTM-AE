...
train_dataset_x = pd.read_csv('hanagwahak_train_x.csv', index_col=0)
...
val_dataset_x = pd.read_csv('hanagwahak_val_x.csv', index_col=0)
...
for j in range(0,5):
    ...
    #save model
    MODEL_PATH = 'hanagwahak_model.pth'
    torch.save(model, MODEL_PATH)

    #bring test data
    test_dataset_x = pd.read_csv('hanagwahak_test_x.csv', index_col=0)
    test_dataset = torch.tensor(test_dataset_x.values)
    test_dataset = test_dataset.unsqueeze(dim=2)
    test_dataset = test_dataset.float()

    y_test = pd.read_csv('hanagwahak_test_y.csv', index_col=0)
