...
# x, y
train = pd.read_csv('split/dataC_train.csv')
x_train = train.loc[:,['UsedPower']] #2차원 dataframe으로
y_train = train.loc[:,['label']]

val = pd.read_csv('split/dataC_val.csv')
x_val = val.loc[:,['UsedPower']] #2차원 dataframe으로
y_val = val.loc[:,['label']]

test=pd.read_csv('split/dataC_test.csv')
...
output_X.to_csv('dataC_train_x.csv')
output_y.to_csv('dataC_train_y.csv')
...
output_X.to_csv('dataC_val_x.csv')
output_y.to_csv('dataC_val_y.csv')
...
output_X.to_csv('dataC_test_x.csv')
output_y.to_csv('dataC_test_y.csv')